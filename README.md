...
Project files
- credit_risk.csv
  Input dataset. The script expects a target column named id where 0 means accepted and 1 means rejected.
- model.py
  Full reproducible script. Running this script produces all deliverables and saves them to the output folder.
- output/
  Generated artifacts when model.py is run:
  - xgb_model.joblib
  - tuning_results.json
  - cv_train_folds.json
  - test_metrics.json
...
pip install -U pip
pip install xgboost shap lime scikit-learn pandas matplotlib joblib scipy

Run the project
1. Place your dataset in the project folder and name it credit_risk.csv or edit model.py to change DATA_FILE.
2. Run:
python model.py
3. Check the output folder for plots, JSON metrics, and local explanation files.
...
Contact
If you would like a Jupyter notebook version, or a PDF of the executive summary and the comparative analysis formatted for submission, ask and I will produce them.
